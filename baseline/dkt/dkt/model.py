...
class LSTM(nn.Module):
    def __init__(self, args):
        ...
        # #추가 임베딩들 time,qacc,rtime,asselp
        self.embedding_elapsed = nn.Embedding(self.args.n_elapsed + 1, self.hidden_dim // 3)
        self.embedding_qacc = nn.Embedding(self.args.n_Question_acc + 1, self.hidden_dim // 3)
        self.embedding_rtime = nn.Embedding(self.args.n_RepeatedTime + 1, self.hidden_dim // 3)
        self.embedding_asselp = nn.Embedding(self.args.n_ass_elp_x + 1, self.hidden_dim // 3)
    ...
    def forward(self, input):
        test, question, tag, _,elapsed,qacc,rtime,asselp, mask, interaction = input
        ...
        #  추가 임베딩들 time,qacc,rtime,asselp
        embedding_elapsed=self.embedding_elapsed(elapsed) 
        embedding_qacc=self.embedding_qacc(qacc)
        embedding_rtime=self.embedding_rtime(rtime) 
        embedding_asselp=self.embedding_asselp(asselp)
        embed = torch.cat(
            [
                embed_interaction,
                embed_test,
...
class LSTMATTN(nn.Module):
    ...
    def forward(self, input):
        ...
        # test, question, tag, _, mask, interaction = input
        test, question, tag, _,time,qacc,rtime,asselp, mask, interaction = input
        ...
        # )
        #추가 임베딩들 time,qacc,rtime,asselp
        embedding_elapsed=self.embedding_elapsed(time) 
        embedding_qacc=self.embedding_qacc(qacc)
        embedding_rtime=self.embedding_rtime(rtime) 
        embedding_asselp=self.embedding_asselp(asselp)
...
class Saint(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.enc_comb_proj = nn.Linear((self.hidden_dim//3)*7, self.hidden_dim)
        ...
        self.dec_comb_proj = nn.Linear((self.hidden_dim//3)*8, self.hidden_dim)
...
class EncoderEmbedding(nn.Module):
    def __init__(self, n_exercises, n_categories,n_tests, n_dims, seq_len,n_Question_acc,n_RepeatedTime,n_ass_elp_x):
        ...
        self.test_embed=nn.Embedding(n_tests, n_dims)
        self.position_embed = nn.Embedding(seq_len, n_dims)

        self.embedding_qacc = nn.Embedding(n_Question_acc, n_dims)
        self.embedding_rtime = nn.Embedding(n_RepeatedTime, n_dims)
        self.embedding_asselp = nn.Embedding(n_ass_elp_x, n_dims)




    def forward(self, exercises, categories,tests,qaccs,rtimes,asselps):
        ...
        t = self.test_embed(tests)
        seq = torch.arange(self.seq_len, device=Config.device).unsqueeze(0)
        p = self.position_embed(seq)

        embedding_qacc=self.embedding_qacc(qaccs)
        embedding_rtime=self.embedding_rtime(rtimes) 
        embedding_asselp=self.embedding_asselp(asselps)
        return p + c + e + t + embedding_qacc + embedding_rtime + embedding_asselp
...
class PlusSAINTModule(nn.Module):
    def __init__(self,args):
        ...
        self.encoder_embedding = EncoderEmbedding(n_exercises=self.args.n_questions + 1,
                                                  ...
                                                  n_tests=self.args.n_test+1,
                                                  n_dims=self.args.hidden_dim,
                                                  seq_len=self.args.max_seq_len,
                                                  n_Question_acc=self.args.n_Question_acc+1,
                                                  n_RepeatedTime=self.args.n_RepeatedTime+1,
                                                  n_ass_elp_x=self.args.n_ass_elp_x+1
                                                  )
    ...
    def forward(self,input ):
        ...
        batch_size = interaction.size(0)
        enc = self.encoder_embedding(
            exercises=question, categories=tag,tests=test,qaccs=qacc,rtimes=rtime,asselps=asselp)
        dec = self.decoder_embedding(responses=interaction)
        # elapsed_time = x["Timestamp"].unsqueeze(-1).float()
        elapsed_time = elapsed.to(self.device)
